...
public class Grafo
{
    ...
    public String masCercano(String v1)
    {
        int posicion = busquedaIndex(v1);
        int pMenorPeso = 30;
        
        for (int i = 0; i < vertices.length; i++)
        {
            if (matrizPesos[posicion][i] < pMenorPeso && matrizPesos[posicion][i] != 0)
            {
                pMenorPeso = i;
            }
        }
        
        return vertices[pMenorPeso];
        
    }
    
    public void nodosProximos(String v)
    {
        int posicion = busquedaIndex(v);
        
        System.out.println("Nodos Proximos:");
        
        for (int i = 0; i < vertices.length; i++)
        {
            if (matrizPesos[posicion][i] != 0)
            {
